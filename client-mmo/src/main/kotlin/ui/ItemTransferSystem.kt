...
class ItemTransferSystem(private val game: MMOGame) {
    ...
    fun setItemManager(itemManager: ItemManager) {
        ...
        println("ItemManager podłączony do ItemTransferSystem")
    }
    ...
    fun selectItem(itemId: String, itemName: String, sourceType: String, sourceSlot: Int) {
        ...
        println("Wybrano item: $itemName z $sourceType slot $sourceSlot")
    }
    ...
    fun clearSelection() {
        selectedItem?.let {
            println("Anulowano przenoszenie: ${it.itemName}")
        }
        selectedItem = null
    }
    ...
    fun moveItem(toType: String, toSlot: Int): ItemInTransfer? {
        ...
        if (item != null) {
            println("Przenoszenie ${item.itemName} z ${item.sourceType}:${item.sourceSlot} do $toType:$toSlot")
            ...
            if (manager != null) {
                when {
                    ...
                    item.sourceType == "INVENTORY" && toType == "INVENTORY" -> {
                        // W obrębie inventory
                        println("DEBUG: Wywołuję moveItemWithinInventory(${item.sourceSlot}, $toSlot)")
                        val success = manager.moveItemWithinInventory(item.sourceSlot, toSlot)
                        if (!success) {
                            println("BŁĄD: moveItemWithinInventory zwróciło false")
                        } else {
                            // TYMCZASOWE rozwiązanie - odśwież inventory po krótkim opóźnieniu
                            println("DEBUG: Planuję odświeżenie inventory za 100ms")
                            // Można dodać timer lub po prostu przeładować inventory z serwera
                        }
                    }
            ...
            clearSelection()
            println("DEBUG: Selection cleared po wykonaniu moveItem")
        }
        return item
    }
