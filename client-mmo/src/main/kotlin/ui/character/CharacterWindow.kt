...
class CharacterWindow(private val game: MMOGame) {
    ...
    // Dodaj metodę setTransferSystem
    ...
    fun render(x: Float, y: Float, width: Float, height: Float) {
        ...
        // Render tabs
        renderTabs(x, y + height - tabHeight, width, tabHeight)

        // Render active tab content
        renderActiveTabContent(x, y, width, height - tabHeight)
    }
    ...
    private fun renderTabs(x: Float, y: Float, width: Float, tabHeight: Float) {
    ...
    private fun renderActiveTabContent(x: Float, y: Float, width: Float, height: Float) {
        when (selectedTab) {
            0 -> characterEquipment.render(x, y, width, height)
            1 -> characterStats.render(x, y, height)
        }
    }
